...
import type { RunFunctionRequest, RunFunctionResponse, Resource } from '../proto/run_function.js';
...
describe('setDesiredCompositeStatus', () => {
    ...
    it('should handle complex nested status objects', () => {
        ...
        expect(result.desired?.composite?.resource?.status).toEqual(status);
        expect(result.desired?.composite?.resource?.status?.atProvider?.connectionPool?.maxConnections).toBe(100);
    });
...
describe('setDesiredResources', () => {
    it('should set resources from unstructured objects', () => {
        ...
        expect(result.desired?.resources?.['my-bucket']?.resource?.spec?.forProvider?.region).toBe('us-west-2');
    });
    ...
    it('should handle complex nested resource structures', () => {
        ...
        expect(result.desired?.resources?.['complex-resource']?.resource?.spec?.template?.spec?.containers[0]?.name).toBe('app');
    });
